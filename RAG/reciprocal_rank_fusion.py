...
'''
Contains the retrieving mechanism from multiple indexing algorithms such as Lexical and Vector Index.
The technique to merge search results is itself known as RECIPROCAL RANK FUSION

This file contains the basic implementation of the retreiver class that will merge the seach results from BM25 and VectorDB results.
'''
...
# chunks by section
def chunks_by_section(document_text):
    pattern = r"\n##"
...
def generate_embedding(text, model="voyage-3-large", input_type="query"):
    ...
    chunks = text if is_list else [text]
    results = voyage_client.embed(chunks, model=model, input_type=input_type)
    return results.embeddings if is_list else results.embeddings[0]
...
# creating chunks
chunks = chunks_by_section(text)

# creating stores
vector_store = VectorIndex(embedding_fn=generate_embedding)
lexical_store = BM25Index()

# Retreiver 
retreiver = Retriever(vector_store, lexical_store)

# adding the chunks to the retreiver
retreiver.add_documents([{"content": chunk for chunk in chunks}])

results = retreiver.search("What happened with INC-2023-Q3-011?")
