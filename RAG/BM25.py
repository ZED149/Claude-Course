...
'''
This file contains the basic implementation of the BM25 search.
BM25 means Best Match 25, and 25 is represents the 25th variation of the improved formula

Workflow
1. Reads the file from the system
2. Divide the file into chunks using chunks by section
3. Store chunks into the BM25
4. Ask user for query
5. Search the store
6. if matched, prints to the screen
'''
...
# addiing each chunk to the store
for chunk in chunks:
    store.add_document({"content": chunk})

# user query
user_query = "What is the Table of Contents of the file."

# searching the store
matched = store.search(query_text=user_query, k=1)

# printing
for doc, distance in matched:
    print(distance, "\n", doc["content"], "\n-------\n")